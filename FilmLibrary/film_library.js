...
function FilmLibrary() {
    ...
    this.print = () => {
        this.filmLibrary
            .forEach((film) => {
                console.log(`Id: ${film.id}, Title: ${film.title}, Favourite: ${film.favourites}, Watch date: ${film.date}, Score: ${film.rating}`)
            })
    };
...
flib.getAll().then((list) => {
    console.log(list);
});
flib.getAllFavourite().then((list) => {
    console.log(list);
});
flib.getWatchedToday().then((list) => {
    console.log(list);
});
flib.getWatchedEarlier("2022-03-20").then((list) => {
    console.log(list);
});
flib.getRatedEqualOrMoreThan(2).then((list) => {
    console.log(list);
});
flib.getTitle("21 Grams").then((list) => {
    console.log(list);
});
